...
for i=1:1:length(disc_u)-1
    ...
    if mex_speedup==1
    results=function_sum_results(M,N,results,fourier_1D_u,fourier_1D_v);
    else
...
if mex_speedup==1
    ...
    result=asr_fourier_2D_mex(n_x,n_y,results);
else
...
%{
    for m=-M:M
    for j=-M:M
        result(((m+M)*(2*N+1)+1):((m+M+1)*(2*N+1)),((j+M)*(2*N+1)+1):((j+M+1)*(2*N+1)))=results(:,(((m-j)+2*M)*(2*N+1)+1):(((m-j)+2*M+1)*(2*N+1)));
    end
end 
 %}
end