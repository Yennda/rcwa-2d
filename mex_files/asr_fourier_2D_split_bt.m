...
for i=1:1:length(disc_v)-1
    ...
    if use_grid_asr_speed_up==1
        if i==1
            asr_function_only_u=asr_fourier_only_asr(disc_x,disc_u,2*M,d_u,G);
            fourier_1D_u=zeros(1,4*M+1);        
            fourier_1D_u=asr_fourier(f(i,:),disc_x,disc_u,2*M,d_u,G,use_grid_asr_speed_up,asr_function_only_u);
        else
            fourier_1D_u=zeros(1,4*M+1);        
            fourier_1D_u=asr_fourier(f(i,:),disc_x,disc_u,2*M,d_u,G,use_grid_asr_speed_up,asr_function_only_u);
        end
        ...
        fourier_1D_u=zeros(1,4*M+1);
        fourier_1D_u=asr_fourier(f(i,:),disc_x,disc_u,2*M,d_u,G,use_grid_asr_speed_up,0);
    end
    
    f_toep=zeros(2*M+1,2*M+1);
    
    for i1=-M:1:M
        for i2=-M:1:M
            f_toep(i1+M+1,i2+M+1)=fourier_1D_u(i1-i2+2*M+1);
        end
    end
    
    inv_f_toep=zeros(2*M+1,2*M+1);
    ...
    fourier_1D_v=asr_fourier(f_i_v,disc_i_y,disc_i_v,2*N,d_v,G,use_grid_asr_speed_up,0);
    
    if mex_speedup==1
        inv_f_toep=inv_f_toep+1e-100*1i;
        fourier_1D_v=fourier_1D_v+1e-100*1i;
        result=result+1e-100*1i;
        result=asr_fourier_2D_split_bt_mex(M,N,result,inv_f_toep,fourier_1D_v);
    else        
