...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    result_r = mxGetPr(prhs[2]);
    result_i = mxGetPi(prhs[2]);
    
    fourier_1d_u_r = mxGetPr(prhs[3]);
    fourier_1d_u_i = mxGetPi(prhs[3]);
    
    fourier_1d_v_r = mxGetPr(prhs[4]);
    fourier_1d_v_i = mxGetPi(prhs[4]);
    
    plhs[0] = mxCreateDoubleMatrix(2*M+1, 2*N+1, mxCOMPLEX);
