...
incid_2((n_y-1)/2+(n_x-1)*n_y/2+1)=input_1_2/sqrt(normalize_2);
incid_2((n_y-1)/2+(n_x-1)*n_y/2+1+n_sq)=input_2_2/sqrt(normalize_2);
%---
...
if measurement==0
    ...
    m=-N_X:1:N_X;
    n=-N_Y:1:N_Y;
    diff_orders=zeros(n_sq,3);
    for i=1:1:length(m)    
        for j=1:1:length(n)        
            r_index=1+(n(j)+N_Y)+(2*N_Y+1)*(m(i)+N_X);        
            diff_orders(r_index,:)=[r_index,m(i),n(j)];    
        end        
    end
