...
if factorization_Li==1
    ...
    exist_loop2=exist('loop_index2','var');
    
    if exist_loop2==1
        % E, E_inv are stored in memory
    elseif exist_loop2==0        
        if use_fftw==1        
...
if zeta==0
    if factorization_Li==1
        ...
        %
        %inv_E_inv=inv(E_inv);
        %G=[-alpha*beta+(mu*k_0^2).*(NN_x*E*NN_y-NN_x*inv_E_inv*NN_y), alpha*alpha+(mu*k_0^2).*(-E+(NN_y*E*NN_y-NN_y*inv_E_inv*NN_y));
        %    (mu*k_0^2).*(E-(NN_x*E*NN_x-NN_x*inv_E_inv*NN_x))-beta*beta, alpha*beta-(mu*k_0^2).*(NN_y*E*NN_x-NN_y*inv_E_inv*NN_x)];
