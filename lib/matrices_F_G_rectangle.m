...
if factorization_Li==1
    ...
    % it causes errors
    bt_E=asr_fourier_2D_split_tb(disc_x,disc_y,disc_x,disc_y,eps_mat,N_X,N_Y,1,mex_speedup,use_grid_asr_speed_up); % based on Camfr (camfr.sourceforge.net) Fourier procedure
    tb_E=asr_fourier_2D_split_bt(disc_x,disc_y,disc_x,disc_y,eps_mat,N_X,N_Y,1,mex_speedup,use_grid_asr_speed_up); % based on Camfr (camfr.sourceforge.net) Fourier procedure

    % wrong
    %wrong_bt_E=asr_fourier_2D_split_tb(disc_x,disc_y,disc_x,disc_y,eps_mat,N_X,N_Y,1,mex_speedup,use_grid_asr_speed_up); % based on Camfr (camfr.sourceforge.net) Fourier procedure
    %wrong_tb_E=asr_fourier_2D_split_bt(disc_x,disc_y,disc_x,disc_y,eps_mat,N_X,N_Y,1,mex_speedup,use_grid_asr_speed_up); % based on Camfr (camfr.sourceforge.net) Fourier procedure

    %tb_E=(tb_E+wrong_tb_E)/2;
    %bt_E=(bt_E+wrong_bt_E)/2;
