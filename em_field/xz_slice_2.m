...
for z_points_loop=1:1:length(z_points)
    ...
    elseif plot_field_component==7 % E_1
        ...
        H_2_harmonics=H_2_plus_eigvec*eval(['gamma_2_' num2str(matice_stejnosti_final(xz_slice_section_to_view,3))])*(-X_plus_u_plus-X_minus_d_minus);
        E_1_harmonics=E_1_plus_eigvec*(X_plus_u_plus+X_minus_d_minus);
        E_2_harmonics=E_1_harmonics; %E_2_plus_eigvec*(X_plus_u_plus+X_minus_d_minus);
    end
    ...
    elseif plot_field_component==7 % E_1
        E_1(:,y_p)=sum(bsxfun(@times,(E_1_harmonics2*exp_alpha_y'),exp_alpha_x_2'));
        %E_1(100:300)=E_1(100:300)./(0.22-6.71*1i)^2;
    end
    %%}
    ...
    elseif plot_field_component==7
        if z_points_loop==1
            row_epsilon=get_eps_row(x_points*1e6,xz_slice_coordinate_z,coordinate_united_x,thickness,eps_xz_slice);
            old_xz_slice_section_to_view=xz_slice_section_to_view;
        elseif old_xz_slice_section_to_view==xz_slice_section_to_view
        else
            row_epsilon=get_eps_row(x_points*1e6,xz_slice_coordinate_z,coordinate_united_x,thickness,eps_xz_slice);
            old_xz_slice_section_to_view=xz_slice_section_to_view;
        end
        row_epsilon=reshape(row_epsilon,length(row_epsilon),1);
        %E_1=0*E_1+1;
        E_1=E_1./row_epsilon;
