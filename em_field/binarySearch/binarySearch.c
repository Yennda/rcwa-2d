...
void mexFunction( int nlhs, mxArray *plhs[],
    ...
    if (dataIsDouble) {
        pdData  = mxGetPr(prhs[0])-1; 
    } else {
        pfData  = ((float*) mxGetData(prhs[0])) -1; 
    }
    N = nrows * ncols;
    ...
    if (itemIsDouble) {
        ...
        pfItems  = ((float*) mxGetData(prhs[1])) -1; 
    }        
    nItems = nrows * ncols;
    ...
    if (nrhs >= 4) {
        pArg = prhs[3];
        if (mxIsEmpty(pArg)) {            
            dirIfNotFound == DIR_IF_NOT_FOUND_DEFAULT; 
        } else if (mxIsChar(pArg)) {  
            pArgStr = mxArrayToString(pArg);
