...
z_point_1=-sum(thickness);
z_point_2=0;

z_points=z_point_1:(z_point_2-z_point_1)/resolution_z:(z_point_2-1/resolution_z);
...
%{
psi=0;      			
v_x=cos(psi)*cos(theta)*cos(phi)-sin(psi)*sin(phi);  
v_y=cos(psi)*cos(theta)*sin(phi)+sin(psi)*cos(phi);  
v_z=-cos(psi)*sin(theta);  
%}

psi=pi/2;
u_x_TE=cos(psi)*cos(theta)*cos(phi)-sin(psi)*sin(phi);  
u_y_TE=cos(psi)*cos(theta)*sin(phi)+sin(psi)*cos(phi);  
...
for z_points_loop=1:1:length(z_points)
    ...
    if plot_field_component==1 % E_1    
        ...
        E_1_harmonics1=reshape(E_1_harmonics1_v',n_x,n_x)';   
    
        E_1_harmonics_0=u_x.*exp(-1i*z_plus*gamma_p00);
    elseif plot_field_component==2 % E_2    
        ...
        E_2_harmonics1=reshape(E_2_harmonics1_v',n_x,n_x)';    
    
        E_2_harmonics_0=u_y.*exp(-1i*z_plus*gamma_p00);
    elseif plot_field_component==3 % E_3    
        ...
        E_3_harmonics1=reshape(E_3_harmonics1_v',n_x,n_x)';    
    
        E_3_harmonics_0=u_z.*exp(-1i*z_plus*gamma_p00);
    elseif plot_field_component==4 % H_1    
        ...
        H_1_harmonics1=reshape(H_1_harmonics1_v',n_x,n_x)';   
    ...
    exp_alpha_x_0=exp(-1i*alpha_0*yz_slice_coordinate_x);
    ...
    %%{
    if plot_field_component==1 % E_1
        E_1(x_p,:)=sum(bsxfun(@times,(E_1_harmonics1*exp_alpha_y_2'),exp_alpha_x'));
        E_1_0(x_p,:)=(bsxfun(@times,(E_1_harmonics_0*exp_alpha_y_2_0'),exp_alpha_x_0'));
        E_1(x_p,:)=E_1(x_p,:)+E_1_0(x_p,:);
    elseif plot_field_component==2 % E_2
        E_2(x_p,:)=sum(bsxfun(@times,(E_2_harmonics1*exp_alpha_y_2'),exp_alpha_x'));
        E_2_0(x_p,:)=(bsxfun(@times,(E_2_harmonics_0*exp_alpha_y_2_0'),exp_alpha_x_0'));
        E_2(x_p,:)=E_2(x_p,:)+E_2_0(x_p,:);
    elseif plot_field_component==3 % E_3
        E_3(x_p,:)=sum(bsxfun(@times,(E_3_harmonics1*exp_alpha_y_2'),exp_alpha_x'));
        E_3_0(x_p,:)=(bsxfun(@times,(E_3_harmonics_0*exp_alpha_y_2_0'),exp_alpha_x_0'));
        E_3(x_p,:)=E_3(x_p,:)+E_3_0(x_p,:);
    elseif plot_field_component==4 % H_1
        H_1(x_p,:)=sum(bsxfun(@times,(H_1_harmonics1*exp_alpha_y_2'),exp_alpha_x'));
        H_1_0(x_p,:)=(bsxfun(@times,(H_1_harmonics_0*exp_alpha_y_2_0'),exp_alpha_x_0'));
        H_1(x_p,:)=H_1(x_p,:)+H_1_0(x_p,:);
    elseif plot_field_component==5 % H_2
        H_2(x_p,:)=sum(bsxfun(@times,(H_2_harmonics1*exp_alpha_y_2'),exp_alpha_x'));
        H_2_0(x_p,:)=(bsxfun(@times,(H_2_harmonics_0*exp_alpha_y_2_0'),exp_alpha_x_0'));
        H_2(x_p,:)=H_2(x_p,:)+H_2_0(x_p,:);
    elseif plot_field_component==6 % H_3
        H_3(x_p,:)=sum(bsxfun(@times,(H_3_harmonics1*exp_alpha_y_2'),exp_alpha_x'));
        H_3_0(x_p,:)=(bsxfun(@times,(H_3_harmonics_0*exp_alpha_y_2_0'),exp_alpha_x_0'));
        H_3(x_p,:)=H_3(x_p,:)+H_3_0(x_p,:);
    end
    %%}
...
%{
if ASR_G~=1
    u_coord=x_points.*1E6; % ve skutecnosti je v u-prostoru u
    x_coord=0.*u_coord.*1E6; % alokace
    %x_coord=u_coord;
    
    v_coord=y_points.*1E6; % ve skutecnosti je v v-prostoru u
    y_coord=0.*v_coord.*1E6; % alokace
        
    x_coord=figure_asr_backward_transformation(u_coord,x_coord,disc_x,disc_u,ASR_G);
    y_coord=figure_asr_backward_transformation(v_coord,y_coord,disc_y,disc_v,ASR_G);
    
    x_points=x_coord.*1E-6; % zpetna transformace
    y_points=y_coord.*1E-6; % zpetna transformace
end
%}
...
axes('FontSize',13)
surf(1E6.*y_points,z_points,profile_mode1,'LineWidth',1);
