if exist('get_intensity')==0
    get_intensity=2;
end

if correct_calculation==1 && plot_field_component==2
...
if plot_field_component==7 && get_intensity==1
    epsilon_matrix_slice_2=[];
end
for z_points_loop=1:1:length(z_points)
    ...
    elseif plot_field_component==7 % E_2
        E_2=zeros(length(x_points),length(y_points));
        ...
        E_1_harmonics=E_1_plus_eigvec*(X_plus_u_plus+X_minus_d_minus);
        E_2_harmonics=E_1_harmonics; %E_2_plus_eigvec*(X_plus_u_plus+X_minus_d_minus);
        H_1_harmonics=H_1_plus_eigvec*eval(['gamma_2_' num2str(matice_stejnosti_final(yz_slice_section_to_view,3))])*(-X_plus_u_plus-X_minus_d_minus);
        %{
        H_1=zeros(length(x_points),length(y_points));
        ...
        E_1_harmonics=(1/k_0)*E_1_plus_eigvec*(X_plus_u_plus+X_minus_d_minus);
        E_2_harmonics=(1/k_0)*E_1_harmonics; %E_2_plus_eigvec*(X_plus_u_plus+X_minus_d_minus);
        H_1_harmonics=(1/k_0)*H_1_plus_eigvec*gamma_2_1*(-X_plus_u_plus-X_minus_d_minus);
        %}
    end   
    ...
    elseif plot_field_component==7
        if z_points_loop==1
            row_epsilon=get_eps_row(y_points*1e6,yz_slice_coordinate_z,coordinate_united_y,thickness,eps_yz_slice);
            old_yz_slice_section_to_view=yz_slice_section_to_view;
        elseif old_yz_slice_section_to_view==yz_slice_section_to_view
        else
            row_epsilon=get_eps_row(y_points*1e6,yz_slice_coordinate_z,coordinate_united_y,thickness,eps_yz_slice);
            old_yz_slice_section_to_view=yz_slice_section_to_view;
        end
    ...
    if plot_field_component==7 && get_intensity==1
        epsilon_matrix_slice_2=[epsilon_matrix_slice_2; row_epsilon];
    end
...
shading flat
axis([y_point_b y_point_e z_points(1) z_points(end)]);
