...
nPermCoeffs_x=2*n_y-1;
maxPermCoeff_x=(nPermCoeffs_x-1)/2;
nPermCoeffs_y=2*n_y-1;
maxPermCoeff_y=(nPermCoeffs_y-1)/2;
...
%[alpha_,beta_]=alpha_beta_mex(n_x,n_y,alpha_m,beta_n);    

%break
%[alpha_1,beta_1]=alpha_beta_mex(n_x,alpha_1_m,beta_1_n);
%[alpha_3,beta_3]=alpha_beta_mex(n_x,alpha_3_m,beta_3_n);

alpha=diag(alpha_0+K_1.*kron((-N_X:N_X),ones(1,2*N_Y+1)));
beta=diag(beta_0+K_2.*kron(ones(1,2*N_X+1),(-N_Y:N_Y)));
...
for layer=number_of_layers:-1:1
    if homogenous_layer(layer)==1
        ...
        %eigvec_H(:,:,layer)=(k_0.*I_mat2)*inv(F)*eigvec_E(:,:,layer)*eval(['gamma_2_' num2str(layer)]); % mu*k_0
        clear G
